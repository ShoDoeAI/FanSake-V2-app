...
const ArtistDashboard = () => {
  ...
  const [myUploads, setMyUploads] = useState({ music: [], images: [], videos: [] });
  ...
  const loadMyUploads = async () => {
    try {
      const uploads = await uploadService.getMyUploads();
      setMyUploads(uploads);
      
      // Update recent content to include uploads
      const allContent = [
        ...uploads.music.map(m => ({ ...m, type: 'music' })),
        ...uploads.images.map(i => ({ ...i, type: 'image' })),
        ...uploads.videos.map(v => ({ ...v, type: 'video' }))
  ...
  const ContentCard = ({ content }) => {
    const getContentIcon = () => {
      switch(content.type) {
        ...
        case 'video':
          return <VideoCameraIcon className="h-8 w-8 text-white" />;
        default:
          return <MusicalNoteIcon className="h-8 w-8 text-white" />;
      }
    };
    ...
    const getContentDate = () => {
      const date = content.uploadDate || content.createdAt;
    ...
    return (
      <div className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500">{getContentDate()}</p>
        </div>
        <div className="flex space-x-4 text-sm text-gray-600">
          <div className="flex items-center">
            <EyeIcon className="h-4 w-4 mr-1" />
            <span>{content.views || content.plays || 0}</span>
          </div>
          <div className="flex items-center">
                ...
                {dashboardData?.analytics ? (
                  ...
                  <div className="h-64 flex items-center justify-center bg-gray-50 rounded-lg">
                    <div className="text-center">
                      ...
                      <Button onClick={() => setUploadModalOpen(true)}>Upload Your First Content</Button>
                    </div>
                  </div>
                )}
              </Card.Body>
            ...
            <Card>
              ...
              <Card.Body className="space-y-3">
                <div className="space-y-2">
                  <p className="text-sm font-medium text-gray-700 mb-2">Upload Content</p>
                  <Button 
                    variant="outline" 
